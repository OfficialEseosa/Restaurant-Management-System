...
\begin{itemize}
    ...
    \item \textbf{Cloudinary} --- image uploads compressed server-side and stored
          ...
          database.
    \item \textbf{Spring Security Crypto} --- BCrypt password hashing; no
...
\begin{itemize}
    ...
    \item \textbf{Audit trail.} \texttt{stock\_change\_log} is append-only ---
          ...
          of every manual stock adjustment.
\end{itemize}
...
\begin{description}[style=nextline]
    ...
    \texttt{PATCH /api/orders/\{id\}/status} accepts a JSON body
    \texttt{\{"status": "READY"\}} and transitions an order through
    \texttt{PLACED $\to$ READY $\to$ COMPLETE} (or \texttt{CANCELLED}).
    Invalid status values return HTTP~400. The admin Orders tab displays all
    orders with per-row status dropdowns that call this endpoint.
    ...
    Using the jqwik library, the test suite verifies business invariants over
    hundreds of randomly generated inputs:
    \begin{itemize}
        \item Order placement always snapshots the correct unit price.
        \item Placing an order with a missing or inactive menu item never calls
              \texttt{orderRepository.save} (atomicity guarantee).
        \item Deleting a referenced ingredient always throws
              \texttt{ConflictException} and never calls \texttt{deleteById}.
    \end{itemize}
\end{description}
...
Property testing    & jqwik + Mockito                & 1.9.3 \\
Password hashing    & Spring Security Crypto (BCrypt)& via Spring Boot 4 \\
\bottomrule
\end{tabular}
\end{table}
...
(\texttt{localhost:8080}) using all HTTP methods including \texttt{PATCH}.
...
The API layer (\texttt{src/api/}) is split into five typed Axios modules.
...
\begin{itemize}
    ...
    \item \texttt{orderApi.ts} --- admin order management (list, filter, update
          status).
    \item \texttt{analyticsApi.ts} --- four analytics endpoints with date-range
          parameters.
    \item \texttt{customerOrderApi.ts} --- customer cart checkout
          (\texttt{POST /api/orders/place}).
\end{itemize}
...
Ahmed Cisse
    & Backend testing and validation. Added Bean Validation annotations to
      entities, \texttt{@Valid} controller enforcement, \texttt{ConflictException}
      with global error handler, ingredient delete guard, and the jqwik
...
\textbf{Property-based testing finds edge cases unit tests miss.}
The jqwik tests for \texttt{OrderService} ran hundreds of randomly generated
order requests and exercised scenarios --- such as empty sublist edge cases and
inactive menu item combinations --- that handwritten unit tests would never have
generated.
...
\begin{itemize}
    ...
    \item \textbf{CORS method coverage.} The order status update uses the
          \texttt{PATCH} HTTP method. An initial CORS configuration that only
          listed \texttt{GET}, \texttt{POST}, \texttt{PUT}, and \texttt{DELETE}
          silently blocked all status updates from the browser. Extending the
          allowed methods list resolved the issue.
...
\begin{itemize}
    \item \textbf{JWT authentication.} Replace \texttt{localStorage} role
          storage with signed JWTs and Spring Security filter chains to protect
          admin endpoints at the server level.
...
\begin{enumerate}
    ...
    \item jqwik. \textit{jqwik --- Property-Based Testing in Java}.
          \url{https://jqwik.net}

    \item Mockito. \textit{Mockito Framework Documentation}.
